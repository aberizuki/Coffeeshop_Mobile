...
const HomeScreen = ({ navigation }) => {
  ...
  const handleDelete = (idProduct) => {
    ...
    // ], {cancelable: true});
    axios
      .delete(`http://192.168.1.4:5000/api/v1/products/${idProduct}`)
      .then((res) => {
        setRefetch(!refetch);
        ToastAndroid.show("Berhasil dihapus", ToastAndroid.SHORT);
  ...
  return (
    <>
      <View style={commonStyle.bg}>
        <Text style={[commonStyle.mlBase, style.title]}>
          ...
          <Text style={{ fontSize: 15 }}>
            {/* Hello, {isLogin.value ? isLogin.data.user.username : ""} */}
          </Text>{" "}
          {`\n`}A good coffee is{`\n`}a good day
        </Text>
        <View>
      ...
      <Modal visible={modalDelete.show} transparent={true} animationType="fade">
        <View
          style={{
            flex: 1,
        ...
        >
          ...
          >
            ...
            <View style={[commonStyle.row, { marginTop: 25 }]}>
              <Pressable
                style={{ backgroundColor: "#f4f4f6", flex: 1, padding: 15 }}
                onPress={() => setModalDelete({ show: false, idProduct: "" })}
              >
                <Text style={{ textAlign: "center" }}>No</Text>
              </Pressable>
              <Pressable
                style={{ backgroundColor: "#fe3f56", flex: 1, padding: 15 }}
